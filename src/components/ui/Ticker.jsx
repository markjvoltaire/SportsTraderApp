import {
  StyleSheet,
  View,
  Animated,
  Easing,
  Text,
  useColorScheme,
} from "react-native";
import React, { useEffect, useRef, useState, useMemo } from "react";
import { Spacing } from "../../constants/theme";
import API_BASE_URL from "../../config/api";

const DEFAULT_HEADLINES = [
  "Lakers beat Warriors 120-115",
  "Chiefs advance to Super Bowl",
  "Yankees sign star pitcher",
  "Heat win in overtime thriller",
  "Cowboys dominate division rivals",
  "Celtics extend winning streak",
  "Packers clinch playoff berth",
  "Dodgers trade for All-Star",
];

export default function Ticker({ items }) {
  const isDarkMode = useColorScheme() !== "light";
  const textColor = isDarkMode ? "#FFFFFF" : "#111827";
  const skeletonColor = isDarkMode ? "rgba(255, 255, 255, 0.3)" : "rgba(17, 24, 39, 0.25)";
  const styles = useMemo(() => createStyles(textColor, skeletonColor), [textColor, skeletonColor]);

  const [tickerItems, setTickerItems] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // If items are provided as props, use them directly
    if (items) {
      setTickerItems(items);
      setLoading(false);
      return;
    }

    // Fetch sports markets (events with nested markets)
    const fetchData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/sports-markets`);
        const data = await response.json();

        const events = Array.isArray(data.events) ? data.events : [];
        if (events.length > 0) {
          // Collect all markets from all events where percentage > 0
          const allMarkets = [];

          events.forEach(event => {
            if (event.markets && event.markets.length > 0) {
              const withPrice = event.markets
                .filter(m => m.yesBid)
                .map(market => ({
                  eventTitle: event.title,
                  marketTitle: market.yesSubTitle,
                  price: Math.round(parseFloat(market.yesBid) * 100),
                }))
                .filter(m => m.price > 0);
              // Top 3 per event: sort by price descending, take 3
              const top3 = withPrice
                .sort((a, b) => b.price - a.price)
                .slice(0, 3);
              allMarkets.push(...top3);
            }
          });
          
          // Format for display
          const formattedItems = allMarkets.map(market => 
            `${market.eventTitle}: ${market.marketTitle} ${market.price}%`
          );
          
          setTickerItems(formattedItems);
        }
      } catch (error) {
        console.error('Error fetching ticker data:', error);
        setTickerItems([]);
      } finally {
        setLoading(false);
      }
    };

    fetchData();
  }, [items]);

  const displayItems = items || tickerItems;
  const tickerAnim = useRef(new Animated.Value(0)).current;
  const tickerAnimationRef = useRef(null);

  // 1. DIMENSIONS (Must be consistent for the math to work)
  const ITEM_WIDTH = 320;
  const ITEM_MARGIN = Spacing.xl;
  const TICKER_BOX_WIDTH = ITEM_WIDTH + ITEM_MARGIN;
  const SINGLE_SET_WIDTH = displayItems.length * TICKER_BOX_WIDTH;

  // Speed control: pixels per millisecond (lower = slower)
  const SPEED = 0.04;

  const startTicker = (startValue) => {
    // Stop any current animation
    if (tickerAnimationRef.current) {
      tickerAnimationRef.current.stop();
      tickerAnimationRef.current = null;
    }

    tickerAnim.setValue(startValue);
    const targetValue = -SINGLE_SET_WIDTH;
    const distanceToGo = Math.abs(targetValue - startValue);

    if (distanceToGo < 1) {
      tickerAnim.setValue(0);
      startTicker(0);
      return;
    }

    const duration = Math.max(100, distanceToGo / SPEED);

    tickerAnimationRef.current = Animated.timing(tickerAnim, {
      toValue: targetValue,
      duration: duration,
      easing: Easing.linear,
      useNativeDriver: true,
    });

    tickerAnimationRef.current.start(({ finished }) => {
      if (finished && tickerAnimationRef.current) {
        // When one set finishes, snap back to 0 and loop
        tickerAnim.setValue(0);
        startTicker(0);
      }
    });
  };

  useEffect(() => {
    if (!loading && displayItems.length > 0) {
      startTicker(0);
    }
    return () => tickerAnimationRef.current?.stop();
  }, [displayItems, loading]);

  // Skeleton loader animation
  const skeletonAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    if (loading) {
      const animation = Animated.loop(
        Animated.sequence([
          Animated.timing(skeletonAnim, {
            toValue: 1,
            duration: 1000,
            easing: Easing.inOut(Easing.ease),
            useNativeDriver: true,
          }),
          Animated.timing(skeletonAnim, {
            toValue: 0,
            duration: 1000,
            easing: Easing.inOut(Easing.ease),
            useNativeDriver: true,
          }),
        ])
      );
      animation.start();
      return () => animation.stop();
    }
  }, [loading, skeletonAnim]);

  const skeletonOpacity = skeletonAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0.3, 0.7],
  });

  // Show skeleton loader while loading
  if (loading) {
    return (
      <View style={styles.tickerContainer}>
        <View style={styles.tickerContent}>
          {[...Array(5)].map((_, index) => (
            <View key={index} style={[styles.tickerBox, { width: 320 }]}>
              <Animated.View
                style={[
                  styles.skeletonItem,
                  { opacity: skeletonOpacity, backgroundColor: skeletonColor },
                ]}
              />
            </View>
          ))}
        </View>
      </View>
    );
  }

  // Don't render anything if no items
  if (displayItems.length === 0) {
    return null;
  }

  return (
    <View style={styles.tickerContainer} pointerEvents="none">
      <Animated.View
        style={[
          styles.tickerContent,
          {
            width: SINGLE_SET_WIDTH * 2, // Ensure container fits both sets
            transform: [{ translateX: tickerAnim }],
          },
        ]}
      >
        {/* Render two copies of the data side-by-side */}
        {displayItems.concat(displayItems).map((item, index) => (
          <View key={index} style={[styles.tickerBox]}>
            <Text style={styles.tickerText} numberOfLines={1}>
              {item}
            </Text>
          </View>
        ))}
      </Animated.View>
    </View>
  );
}

const createStyles = (textColor, skeletonColor) =>
  StyleSheet.create({
    tickerContainer: {
      height: 20,
      marginBottom: 1,
      marginTop: 1,
      bottom: 5,
      overflow: "hidden",
      justifyContent: "center",
      marginHorizontal: 15,
      marginVertical: 8,
   
    },
    tickerContent: {
      flexDirection: "row",
      alignItems: "center",
    },
    tickerBox: {
      marginRight: Spacing.xl,
      alignItems: "center",
      justifyContent: "center",
    },
    tickerText: {
      color: textColor,
      fontSize: 15,
      fontWeight: "600",
      fontFamily: "SubwayTickerGrid",
    },
    skeletonItem: {
      height: 14,
      backgroundColor: skeletonColor,
      borderRadius: 4,
      width: "80%",
    },
  });
